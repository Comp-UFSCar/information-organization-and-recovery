...
O tamanho do bloco/cluster do programa foi obtido usando-se funções para descobrir o 
tamanho dos clusters do próprio sistema operacional. Com o retorno dessas funções, o 
tamanho do bloco/cluster foi definido como o valor em bytes do cluster do próprio SO.

Foi necessário o uso de funções diferentes de acordo com o sistema operacional, pois 
o atributo st_blksize da struct stat já definida pela biblioteca `<sys/stat.h>` só 
existia nos sistemas operacionais baseados em unix. Para windows foi utilizada a
